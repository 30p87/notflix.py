...
Invoke-Expression "$Env:USERPROFILE\AppData\Local\Temp\python_install.exe /quiet InstallAllUsers=1 PrependPath=1 Include_test=0"
msiexec /qn /l* C:\node-log.txt /i $Env:USERPROFILE\AppData\Local\Temp\nodejs_install.msi
Invoke-Expression "$Env:USERPROFILE\AppData\Local\Temp\7z_install.exe /S"
Invoke-Expression "$env:ProgramFiles\7-Zip\7z.exe x -o
